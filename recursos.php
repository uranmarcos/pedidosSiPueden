        ...
        <div class="container">
            ...
            <div class="col-12 p-0">
                <div class="breadcrumb">
                    <span class="pointer mr-2" @click="irAHome()">Inicio</span>  -  <span class="ml-2 grey"> Otros recursos </span>
                </div>
            </div>
            <!-- END BREADCRUMB -->
            ...
            <div class="row rowBotones d-flex justify-content-between mb-3" v-if="usuarioAdmin == 'admin'">  
                <button type="button" class="btn boton" @click="showABMRecurso(modalRecurso)" >
            ...
            <!-- START ABM RECURSOS -->
            <div class="contenedorABM" v-if="modalRecurso">    
                <div class="titleABM">
                    NUEVO RECURSO
                </div>
                ...
                <div class="row rowBotones d-flex justify-content-center">
                    ...
                    <!-- <div class="col-sm-12 col-md-5 mt-3">
                        ...
                        <div class="mt-2">
                            ...
                            <input class="form-control" autocomplete="off" maxlength="60" id="nombre" v-model="recurso.nombre">
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-4 mt-3">
                        <div>
                            <label for="nombre">Descripción (*) <span class="errorLabel" v-if="errorDescripcion">{{errorDescripcion}}</span></label>
                            <textarea class="form-control textareaDescripcion" maxlength="700" v-model="recurso.descripcion"></textarea>
                        </div>
                    </div> -->
                ...
                <div class="footerABM" v-if="!confirmRecurso">
                    <button type="button" class="btn botonABM" @click="cancelarModalRecurso">Cancelar</button>
                    <button type="button" @click="crearRecurso" class="btn botonABM">
                        Crear
                    </button>
                </div>
                <div class="footerABM" v-if="confirmRecurso">
                    <button type="button" class="btn boton" :disabled="loadingRecurso" @click="confirmRecurso = false">Cancelar</button>
                    <div class="row rowBotones f-dlex justify-content-center my-3">
                        ¿Confirma la creación del recurso?
                    </div>
                    
                    <button type="button" @click="confirmarRecurso" class="btn boton" v-if="!loadingRecurso">
                        Confirmar
                    </button>
                    <button 
                        class="btn boton"
            ...
            <div class="row rowBotones d-flex justify-content-between">     
                <select class="form-control selectCategoria" @change="page= 1, consultarRecursos()" v-model="categoriaBusqueda">
            ...
            <div class="row mt-6">
                <div class="col-12">
                    <!-- START COMPONENTE LOADING BUSCANDO RECURSOS -->
                    <div class="contenedorLoading" v-if="buscandoRecursos">
                        <div class="loading">
                            <div class="spinner-border" role="status">
                                <span class="sr-only"></span>
                            </div>
                    ...
                    <!-- END COMPONENTE LOADING BUSCANDO RECURSOS -->

                    <div class="contenedorABM py-3" v-if="verCarrito && !buscandoRecursos" id="carrito">    
                        <span class="subtituloCard">RECURSOS PEDIDOS</span>
                        <ul v-for="recurso in recursosPedidos">
                            <li class="itemListado my-2">
                                <span @click="irARecurso(recurso.id)" class="pointer"> {{recurso.nombre}} <br></span>
                            </li>
                        </ul>
                        <hr>
                        <article>
                    ...
                    <div v-if="recursos.length != 0 && !buscandoRecursos" class="row contenedorLibros d-flex justify-content-around">
                        <article class="col-12 col-lg-6" v-for="recurso  in recursos">
                            <div class="row rowCard">
                                <div class="col-6 col-md-4 p-0" :id="'recurso' + recurso.id" >
                                    <div class="imgCard">
                                        <img  :src="retornarImagen(recurso)"/>
                                      
                                        <button type="button" class="btn botonSmallTrash" @click="eliminarRecurso(recurso.id, recurso.nombre)" data-toggle="modal" data-target="#ModalCategoria" v-if="usuarioAdmin == 'admin'">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
                                                <path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5zm3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0V6z"/>
                                                <path fill-rule="evenodd" d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1v1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4H4.118zM2.5 3V2h11v1h-11z"/>
                                            </svg> ELIMINAR RECURSO
                                        </button>

                                        <button type="button" class="btn botonSmallAdd" @click="agregarRecursoPedido(recurso.id, recurso.nombre)" v-if="usuarioAdmin != 'admin' && recursosPedidos.filter(element =>element.id == recurso.id).length == 0">
                                        ...
                                        <button type="button" class="btn botonSmallTrash" @click="eliminarRecursoPedido(recurso.id, recurso.nombre)" v-if="usuarioAdmin != 'admin' && recursosPedidos.filter(element =>element.id == recurso.id).length != 0">
                                        ...
                                        <button type="button" @click="mostrarCarrito" :disabled="verCarrito" class="btn botonSmallAgregado" v-if="usuarioAdmin != 'admin' && recursosPedidos.length != 0">
                                            VER CARRITO({{recursosPedidos.length}})
                                        </button> 
                                      
                                    </div>
                                </div>
                                <div class="col-6 col-md-8 p-0" :id="'descripcion' + recurso.id" >
                                    <div class="descripcionCard">
                                        <div class="tituloLibro">{{recurso.nombre}}</div>
                    ...
                    <div class="contenedorTabla" v-if="recursos.length == 0 && !buscandoRecursos">
                ...
                <div v-if="modalEliminar">
                    <div id="myModal" class="modal">
                        <div class="modal-content p-0">
                            <div class="modal-header  d-flex justify-content-center">
                                <h5 class="modal-title" id="ModalLabel">ELIMINAR RECURSO</h5>
                            </div>

                            <div class="modal-body row d-flex justify-content-center">
                                <div class="col-sm-12 mt-3 d-flex justify-content-center">
                                    ¿Desea eliminar el recurso {{recursoEliminable.nombre}} ?
                                </div>                                
                            </div>


                            <div class="modal-footer d-flex justify-content-between" v-if="!confirmCategorias">
                                <button type="button" class="btn boton" @click="cancelarEliminarRecurso" >Cancelar</button>
                                
                                <button type="button" @click="confirmarEliminarRecurso" class="btn boton" v-if="!eliminandoRecurso">
                                    Confirmar
                                </button>

                                <button 
                                    class="btn boton"
                                    v-if="eliminandoRecurso" 
                                >
                                    <div class="loading">
                                        <div class="spinner-border" role="status">
                                            <span class="sr-only"></span>
    ...
    <script>
        var app = new Vue({
            ...
            mounted () {
                ...
                let recursosPedidos = JSON.parse(localStorage.getItem("recursosPedidos"));
                if (recursosPedidos) {
                    this.recursosPedidos= recursosPedidos;
                }
            ...
            methods:{
                ...
                irARecurso (id) {
                    document.getElementById("recurso"+id).scrollIntoView();
                },
                agregarRecursoPedido(id, nombre) {
                    let recurso = new Object();
                    recurso.id = id;
                    recurso.nombre = nombre;
                    this.recursosPedidos.push(recurso);
                    localStorage.setItem("recursosPedidos", JSON.stringify(this.recursosPedidos))
                },
                eliminarRecursoPedido(id, nombre) {
                    this.recursosPedidos = this.recursosPedidos.filter(element => element.id != id)
                    localStorage.setItem("recursosPedidos", JSON.stringify(this.recursosPedidos))
                    if (this.recursosPedidos.length == 0) {
                ...
                eliminarRecurso (id, nombre) {
                    this.recursoEliminable.id = id;
                    this.recursoEliminable.nombre = nombre;
                    this.modalEliminar = true;
                },
                cancelarEliminarRecurso () {
                    this.recursoEliminable.nombre = null;
                    this.recursoEliminable.id = null;
                    this.modalEliminar = false;
                },
                confirmarEliminarRecurso() {
                    this.eliminandoRecurso = true;
                    let formdata = new FormData();
                    formdata.append("idRecurso", app.recursoEliminable.id);

                    axios.post("funciones/acciones.php?accion=eliminarRecurso", formdata)
                    .then(function(response){    
                        app.eliminandoRecurso = false;
                ...
                subirImagen () {
                    ...
                    this.recurso.archivo = objectURL
                ...
                consultarCantidad () {
                    ...
                    axios.post("funciones/acciones.php?accion=contarRecursos", formdata)
                    .then(function(response){    
                        if (response.data.error) {
                            app.mostrarToast("Error", response.data.mensaje);
                        } else {
                ...
                consultarRecursos() {
                    ...
                    .then(function(response){    
                        app.buscandoRecursos = false;
                        if (response.data.error) {
                            app.mostrarToast("Error", response.data.mensaje);
                        } else {
                            if (response.data.archivos != false) {
                                app.recursos = response.data.archivos;
                                app.recursos.forEach(element => {
                ...
                confirmar () {
                    ...
                    this.recursosPedidos.forEach(element => {                              
                        pedido = pedido + element.nombre+ "; ";
                    });
                    formdata.append("pedido", pedido);
                   
                   
                    axios.post("funciones/acciones.php?accion=enviarPedidoRecursos", formdata)
                    .then(function(response){    
                        if (response.data.error) {
                            app.errorEnvio= true;
                            ...
                            localStorage.removeItem("recursosPedidos")
                        }
                        app.loading = false;
                    }).catch( error => {
                        app.errorEnvio= true;
                ...
                // START MODAL RECURSO
                showABMRecurso (param) {
                    this.modalRecurso = !param
                    if (param) {
                        this.cancelarModalRecurso()
                    }
                },
                cancelarModalRecurso () {
                    this.modalRecurso = false;
                    this.selectAnadir = 0;
                    this.resetErroresNuevoRecurso();
                    this.resetNuevoRecurso();
                },
                processFile(event) {
                    if (event != undefined) {
                        this.archivo = event;
                    ...
                    this.recurso.nombreImagen = event.target.files[0].name
                    this.subirImagen()
                },  
                crearRecurso () {
                    let error = false;
                    this.resetErroresNuevoRecurso();
                    if (this.recurso.archivo == null) {
                    ...
                    if (this.recurso.nombre == null || this.recurso.nombre.trim() == '') {
                        this.errorNombre = "Campo requerido";
                        error = true;
                    } 
                    if (this.recurso.descripcion == null || this.recurso.descripcion.trim() == '') {
                        this.errorDescripcion = "Campo requerido";
                        error = true;
                    } 
                    if (!error) {
                        this.confirmRecurso = true;
                    }
                },
                confirmarRecurso () {
                    app.creandoRecurso = true;
                    let formdata = new FormData();
                    formdata.append("tipo", "recurso");
                    ...
                    formdata.append("archivo", app.recurso.archivo);
                    formdata.append("nombre", app.recurso.nombre);
                    formdata.append("descripcion", app.recurso.descripcion);
                    ...
                    .then(function(response){
                        if (response.data.error) {
                            ...
                            app.modalRecurso = false;
                            app.confirmRecurso = false;
                            app.categorias.forEach(element => {
                                element.checked = false;
                            });
                            app.mostrarToast("Éxito", response.data.mensaje);
                            app.consultarRecursos();
                            app.resetNuevoRecurso();
                        }
                        app.creandoRecurso = false;
                    }).catch( error => {
                        app.creandoRecurso = false;
                        app.mostrarToast("Error", "No se pudo crear el recurso");
                    })
                },
                resetErroresNuevoRecurso() {
                ...
                resetNuevoRecurso() {
                    ...
                    this.recurso.nombre = null;
                    this.recurso.descripcion = null;
                },
                // END FUNCIONES LIBRO
            ...
            watch: {
                verArchivo () {
                    if (this.pdfbase64) {
                        this.recurso.archivo = this.pdfbase64;
                    }
